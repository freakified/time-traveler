...
void time_traveler_overlay_update(WorldClockOverlay *overlay,
  ...
  overlay->expected_rows = current_height > time_traveler_OVERLAY_MAX_ROWS ? time_traveler_OVERLAY_MAX_ROWS : current_height;
  overlay->received_rows = overlay->expected_rows;
  overlay->valid = true;
  
  time_t now = time(NULL);
  SolarPosition solar = time_traveler_solar_position(now);
  ...
  for (uint16_t row = 0; row < overlay->expected_rows; ++row) {
    ...
    time_traveler_daylight_interval(latitude, solar, current_width,
...
bool time_traveler_overlay_is_city_night(const WorldClockOverlay *overlay,
  ...
  if (day_start == time_traveler_OVERLAY_FULL_NIGHT &&
      day_end == time_traveler_OVERLAY_FULL_NIGHT) {
    return true;
  }

  if (day_start == 0 && day_end == time_traveler_OVERLAY_FULL_DAY_SENTINEL) {
    return false;
  }
